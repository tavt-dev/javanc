    ...
    <div className="grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
      <form onSubmit={onSubmit} className="grid gap-4 rounded-md border border-line bg-white p-5 shadow-soft">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <Field label="Name">
            <input name="name" className={inputClass} placeholder="Hieu Cao" value={draft.name ?? ""} onChange={(event) => updateDraft("name", event.target.value)} />
          </Field>
          <Field label={t("profile.titleLabel")}>
            <input name="title" className={inputClass} placeholder="Senior Java Developer" value={draft.title ?? ""} onChange={(event) => updateDraft("title", event.target.value)} />
          </Field>
        </div>
        <div className="grid gap-4 md:grid-cols-2">
        ...
        <Field label={t("profile.objectiveLabel")}>
          <textarea name="objective" className={inputClass} rows={3} placeholder="Professional objective" value={draft.objective ?? ""} onChange={(event) => updateDraft("objective", event.target.value)} />
        </Field>
        ...
        <div className="flex justify-end">
          <Button type="submit" disabled={saving}>
            {saving ? t("profile.saving") : mode === "edit" ? "Update profile" : t("profile.save")}
          </Button>
        </div>
      </form>
      <aside className="scroll-reveal rounded-md border border-line bg-white p-5 shadow-soft">
        ...
        <div className="mt-5 flex items-start justify-between gap-3">
          <div className="min-w-0">
            <h2 className="break-words font-serif text-4xl font-bold leading-tight text-ink">{draft.name || "Profile name"}</h2>
            <p className="mt-2 break-words text-xl font-semibold text-brand">{draft.title || "Profile title"}</p>
          </div>
          <Pill tone="blue">{draft.typeProfile || "Profile"}</Pill>
        </div>
        <div className="mt-4 grid gap-2 rounded-md border border-line bg-white p-3 text-sm text-muted">
          ...
          <span className="flex items-center gap-2">
            ...
            {draft.contact?.email || "Email not set"}
          </span>
          <span className="flex items-center gap-2">
            <Phone className="h-4 w-4 text-brand" />
            {draft.contact?.phone || "Phone not set"}
          </span>
          <span className="flex items-center gap-2">
            <MapPin className="h-4 w-4 text-brand" />
            {draft.contact?.address || "Address not set"}
          </span>
        </div>
        <p className="mt-4 text-sm leading-6 text-muted">{draft.objective || "Profile objective preview appears here."}</p>
        <div className="mt-5 grid gap-3 text-sm text-muted">
          <p><span className="font-semibold text-ink">Education:</span> {draft.education || "Not set"}</p>
          <div>
          ...
          <p><span className="font-semibold text-ink">Experience:</span> {draft.workExperience || "Not set"}</p>
        </div>
      </aside>
      {pendingImageUrl ? (
