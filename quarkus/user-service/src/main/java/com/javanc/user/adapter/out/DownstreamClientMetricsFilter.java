package com.javanc.user.adapter.out;
...
public class DownstreamClientMetricsFilter implements ClientRequestFilter, ClientResponseFilter {
    ...
    public void filter(ClientRequestContext requestContext, ClientResponseContext responseContext) throws IOException {
        ...
        Timer.builder("javanc_downstream_http_client_requests")
                .tags(Tags.of("service", "user-service", "targetService", targetService(requestContext), "method",
                        requestContext.getMethod(), "status", Integer.toString(status), "statusClass",
                        (status / 100) + "xx"))
