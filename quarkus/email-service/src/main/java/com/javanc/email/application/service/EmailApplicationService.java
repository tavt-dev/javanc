...
public class EmailApplicationService {
    ...
    public void sendPasswordResetOtp(VerificationOtpEmailDTO request) {
        if (request == null || blank(request.getTo()) || blank(request.getOtp()) || request.getExpiresInMinutes() <= 0) {
            throw new ApplicationException(ErrorCode.BAD_REQUEST);
        }
        String name = blank(request.getName()) ? "there" : request.getName().trim();
        String subject = "Reset your Javanc password";
        String content = passwordResetOtpTemplate(name, request.getOtp().trim(), request.getExpiresInMinutes());
        mailSenderPort.send(new MailMessage(null, request.getTo().trim(), subject, content, TEXT_HTML));
    }
    ...
    private String passwordResetOtpTemplate(String name, String otp, long expiresInMinutes) {
        return """
                <!doctype html>
                <html lang="en">
                <head>
                  <meta charset="utf-8">
                  <meta name="viewport" content="width=device-width, initial-scale=1">
                  <title>Reset your Javanc password</title>
                </head>
                <body style="margin:0;padding:0;background:#f4f7fb;color:#172033;font-family:Arial,Helvetica,sans-serif;">
                  <table role="presentation" width="100%%" cellspacing="0" cellpadding="0" style="background:#f4f7fb;margin:0;padding:32px 16px;">
                    <tr>
                      <td align="center">
                        <table role="presentation" width="100%%" cellspacing="0" cellpadding="0" style="max-width:560px;background:#ffffff;border-radius:18px;overflow:hidden;border:1px solid #e5ebf3;box-shadow:0 18px 44px rgba(23,32,51,0.10);">
                          <tr>
                            <td style="padding:28px 32px;background:#312e81;color:#ffffff;">
                              <div style="font-size:13px;letter-spacing:0.08em;text-transform:uppercase;font-weight:700;opacity:0.82;">Javanc</div>
                              <h1 style="margin:10px 0 0;font-size:26px;line-height:1.25;font-weight:800;">Reset your password</h1>
                            </td>
                          </tr>
                          <tr>
                            <td style="padding:32px;">
                              <p style="margin:0 0 16px;font-size:16px;line-height:1.6;color:#334155;">Hi %s,</p>
                              <p style="margin:0 0 24px;font-size:16px;line-height:1.6;color:#334155;">Use this one-time code to set a new password for your Javanc account.</p>
                              <div style="margin:0 0 24px;padding:22px 20px;text-align:center;background:#eef2ff;border:1px solid #c7d2fe;border-radius:14px;">
                                <div style="margin:0 0 8px;font-size:12px;letter-spacing:0.12em;text-transform:uppercase;color:#4338ca;font-weight:700;">Reset code</div>
                                <div style="font-size:34px;line-height:1.2;letter-spacing:0.22em;font-weight:800;color:#312e81;font-family:'Courier New',Courier,monospace;">%s</div>
                              </div>
                              <p style="margin:0 0 18px;font-size:14px;line-height:1.6;color:#475569;">This code expires in <strong>%d minutes</strong>. For your security, do not share this code with anyone.</p>
