...
public class RateLimitMetrics {
    ...
    public void decision(RateLimitDecision decision) {
        ...
        if (!decision.allowed() && decision.mode() == RateLimitMode.SHADOW) {
            Counter.builder("javanc_rate_limit_shadow_would_block_total")
                    .tags(Tags.of("service", "gateway-service", "policy", decision.policy().name()))
                    .register(meterRegistry)
                    .increment();
        }
        if (!decision.allowed()) {
            LOG.warnf("rate_limit_decision service=gateway-service requestId=%s policy=%s mode=%s outcome=%s identityType=%s",
    ...
    public void redisError(String operation) {
        Counter.builder("javanc_rate_limit_redis_error_total")
                .tags(Tags.of("service", "gateway-service", "operation", operation))
                .register(meterRegistry)
                .increment();
    }
